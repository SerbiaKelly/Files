...
function this.Awake(obj)
    ...
    atlas_PHZ = gameObject.transform:Find('atlasAttach_PHZ');
    atlas_XPLP = gameObject.transform:Find('atlasAttach_XPLP');
    -- local card = gameObject.transform:Find('card');
    -- for i=1,cardNum do
...
local FUC
function this.WhoShow(xipaiData)
    FUC=xipaiData.finishedFun;
    this.initCards()
    if xipaiData.needChangeAtlas then 
        this.ChangeAtlasAndSpriteName(xipaiData.atlasName,xipaiData.spriteName);
    else
        this.ChangeAtlasAndSpriteName("Game","Card_Big_000");
    end
    StartCoroutine(this.qiePai)
end

function this.ChangeAtlasAndSpriteName(atlasName,spriteName)
    local newAtlas = nil;
    if atlasName == "Game" then 
        newAtlas = atlas_PHZ:GetComponent("UISprite").atlas;
    elseif atlasName == "ingamexplp" then 
        newAtlas = atlas_XPLP:GetComponent("UISprite").atlas;
    end
    for i=1,#cards do
        local card = cards[i];
        local cardUISprite = card:GetComponent("UISprite");
        cardUISprite.atlas = newAtlas;
        cardUISprite.spriteName = spriteName;
    end
end
