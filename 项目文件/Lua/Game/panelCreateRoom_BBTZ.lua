...
function this.Awake(go)
    ...
    local BBTZPanel = gameObject.transform:Find("BBTZPanel")
...
function this.OnEnable()
	...
	BBTZ_View.ToggleTrusteeshipOne.parent.gameObject:SetActive(true)
	BBTZ_View.ToggleTrusteeshipAll:GetComponent('UIToggle'):Set(BBTZ_Data.trusteeshipDissolve == false)
...
function this.OnClickOk(go)
	...
	local rule = proxy_pb.PCreateRoom();

    local settings = {}
    settings.rounds = 2--BBTZ_Data.rounds
    settings.size = BBTZ_Data.size
    settings.paymentType = BBTZ_Data.paymentType
    ...
    rule.gameType = 5
    rule.settings = json:encode(settings)
    msg.body = rule:SerializeToString()
