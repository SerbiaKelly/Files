...
panelStageClear_hnzzm = {}
local this = panelStageClear_hnzzm
...
function this.Awake(obj)
    ...
    for i=1,4 do
		...
		score_pao[i] 		= gameObject.transform:Find('Group/playerGrid/Player'..i..'/decScore/Label_pao/Label_pao')
    end
...
function this.WhoShow(data)
	...
	if RoundData.isInGame then 
		uiLayer.HNZZM_Logic.connect.IsZanTing = false;
	end
...
function this.onGetXiPaiResult(msg)
    ...
    if b.seat== RoundData.mySeat then
        ...
        if b.code==83 then
			...
			if RoundData.isInGame then
				uiLayer.HNZZM_Logic.hasStageClear = false;
			end
...
function this.Refresh()
	...
	for i=1,#RoundData.data.players do
		...
		local logic = uiLayer.HNZZM_Logic == nil and uiLayer.HNZZM_RePlayLogic or uiLayer.HNZZM_Logic;
		...
		score_pao[i]:GetComponent('UILabel').text 	= player.endPaoScore;

		paiXing[i].gameObject:SetActive(true);
		
		paiXing[i]:GetComponent("UILabel").text = HNZZM_Tools.GetMJWinResult(player.huScore);
		--设置庄家
	...
	for i=1,#RoundData.data.players do
		...
		for j=0,playerGridHand[i].childCount-1 do
			if j < #player.mahjongs then
				...
				-- end
				if uiLayer.roomData.setting.takeHun and this.CheckHunPai(player.mahjongs[j+1]) then
					item:Find("mark").gameObject:SetActive(true);
					item:Find("mark/markLabel"):GetComponent("UILabel").text = "混";
				else
					item:Find("mark").gameObject:SetActive(false);
				end
			else
		...
		if #player.huItems > 0 then
			...
			for j = 1,#player.huItems do
				...
				if uiLayer.roomData.setting.takeHun and this.CheckHunPai(player.huItems[j].huMahjong) then
					hupai.transform:Find("mark").gameObject:SetActive(true);
					hupai.transform:Find("mark/markLabel"):GetComponent("UILabel").text = "混";
				else
					hupai.transform:Find("mark").gameObject:SetActive(false);
				end
			end
