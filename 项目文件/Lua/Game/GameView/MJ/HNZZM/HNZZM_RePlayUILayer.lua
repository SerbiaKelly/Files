...
require("HNZZM_RePlayEventBind");
require("HNZZM_Tools");
require("const");
require("effManger");
local gameUIObjects         = require("HNZZM_GameObjs");
local json                  = require("json");
HNZZM_RePlayUILayer         = class();
...
function HNZZM_RePlayUILayer:ctor(gameObject,HNZZM_RePlayLogic,game_pb)
    this                        = self;
    self.HNZZM_RePlayLogic      = HNZZM_RePlayLogic;
    self.nmm_pb                 = game_pb;
...
function HNZZM_RePlayUILayer:GetGameLogic()
    return self.HNZZM_RePlayLogic;
end


function HNZZM_RePlayUILayer:GetUIS(gameObject)
...
function HNZZM_RePlayUILayer:GetPlayersUI(gameObject)
    local maxPlayerSize = self.HNZZM_RePlayLogic:GetMaxPlayerSize();
    ...
    for i=1,maxPlayerSize do
        ...
        local HNZZM_GridTool        = HNZZM_GridTool.New(gridHand,gridXi,gridThrow,gridMo,chuPaiShow,(i-1),nil,{uiLayer = self});
        local playerView            = HNZZM_Player.New(obj.gameObject,gameObject);
        local RoundDetail           = {};
        RoundDetail.openUserCard    = true;
        RoundDetail.isLord          = true;
        playerView:SetGameLogic(self.HNZZM_RePlayLogic):SetMJGridTool(HNZZM_GridTool,self.HNZZM_RePlayLogic,self);
...
function HNZZM_RePlayUILayer:GetButtons(gameObject)
...
function HNZZM_RePlayUILayer:GetRoomInfoUI(gameObject)
...
function HNZZM_RePlayUILayer:GetESWNUI(gameObject)
    gameUIObjects.dipai 			    = gameObject.transform:Find('DiPai');
    for i=1,self.HNZZM_RePlayLogic:GetMaxPlayerSize() do
        gameUIObjects.ActivePlayerESWN[i] = gameObject.transform:Find("DiPai/DeskTimerIndex"..i);
    end
end

function HNZZM_RePlayUILayer:InitUILayer(roomData)
    ...
    HNZZM_RePlayEventBind.Init(self,gameUIObjects,self.HNZZM_RePlayLogic);--绑定事件
    self:InitControls();
    self:StartPlay();--开始播放
...
function HNZZM_RePlayUILayer:InitRoomInfo(roomData)
    --获取主题颜色
    ...
    coroutine.start(RefreshPhoneState,self.gameObject,self.HNZZM_RePlayLogic,gameUIObjects.batteryLevel,gameUIObjects.network);
    ...
    gameUIObjects.roomSetting:Find('Label'):GetComponent('UILabel').text    = GetHNZZMRuleText(roomData.setting,false,false);
    ...
    --游戏开始时是否显示蹲拉跑
    -- print("mySeat:"..self.HNZZM_RePlayLogic.mySeat);
    -- print("completeChooce:"..self.HNZZM_RePlayLogic:GetPlayerDataBySeat(self.HNZZM_RePlayLogic.mySeat).completeChooce);
    -- print("bankerSeat:"..self.roomData.bankerSeat)
    -- if self.HNZZM_RePlayLogic:GetPlayerDataBySeat(self.HNZZM_RePlayLogic.mySeat).completeChooce == 1 then 
    --     PanelManager.Instance:ShowWindow("panelBetOn",{seat = self.HNZZM_RePlayLogic.mySeat,isBanker = self.HNZZM_RePlayLogic:IsBanker()})
    -- end 
...
function HNZZM_RePlayUILayer:InitPlayerInfo(roomData)
    ...
    DestroyRoomData.playerData      = self.HNZZM_RePlayLogic:GetPlayerDatas();
    DestroyRoomData.mySeat 		    = self.HNZZM_RePlayLogic.mySeat;

end

function HNZZM_RePlayUILayer:InitAllGrids(roomData)
    --隐藏所有的grid
    ...
    --显示玩家的操作
    -- print("mySeat:"..tostring(self.HNZZM_RePlayLogic.mySeat));
    -- print("playerdatas.length:"..self.HNZZM_RePlayLogic:GetPlayerDataLength());
    local myData = self.HNZZM_RePlayLogic:GetPlayerDataBySeat(self.HNZZM_RePlayLogic.mySeat);
    -- print("myoperations.length:"..#myData.operations);
    self:RefreshOperationSend(myData.operations,myData);
    --处理听牌
    local myData        = self.HNZZM_RePlayLogic:GetPlayerDataBySeat(self.HNZZM_RePlayLogic.mySeat);
    local myRealIndex   = self.HNZZM_RePlayLogic:GetCorrectUIIndex(self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(myData.seat));
    local myPlayerView  = gameUIObjects.playerViews[myRealIndex];
   

end

function HNZZM_RePlayUILayer:SetGameTypePos()
...
function HNZZM_RePlayUILayer:InitESWNInfo(roomData)
    gameUIObjects.dipai.gameObject:SetActive(false);
    self:SetDipaiNum(roomData.surplus);
    self:SetRoundNum(roomData.round);
    gameUIObjects.dipai.gameObject:SetActive(HNZZM_PlayLogic.NeedShowDiPai(self.roomData,self.HNZZM_RePlayLogic));
    gameUIObjects.roomRound.gameObject:SetActive(HNZZM_PlayLogic.NeedShowDiPai(self.roomData,self.HNZZM_RePlayLogic));
    self:SetActivePlayer(self.roomData);
end


function HNZZM_RePlayUILayer:SetPlayerPos(roomData)
    --设置座位位置
    for i=1,#gameUIObjects.playerViews do
        local playerView =  gameUIObjects.playerViews[i];
        playerView:SetPos(roomData.state  ~= self.nmm_pb.GAMING and (self.HNZZM_RePlayLogic.totalSize < roomData.setting.size or roomData.round == 1) );
...
--刷新所有grid
function HNZZM_RePlayUILayer:RefreshAllGrid( )
    -- body
    -- print("RefreshAllGrid------------->")
    self.HNZZM_RePlayLogic:ForeachPlayerDatas(function(playerView,_) 
...
--刷新手牌
function HNZZM_RePlayUILayer:RefreshAllGridHand()
    -- print("RefreshAllGridHand----------->")
    self.HNZZM_RePlayLogic:ForeachPlayerDatas(function(playerView,playerData) 
        ...
        playerView.hnzzm_GridTool:GridHandPlatesDown(playerData.mahjongs,playerData.mahjongs);
    end);
end

--刷新喜牌
function HNZZM_RePlayUILayer:RefreshAllGridXi()
...
--刷新带出去的牌
function HNZZM_RePlayUILayer:RefreshAllGridThrow()
...
--刷新摸牌
function HNZZM_RePlayUILayer:RefreshAllGridMoPai()
    -- print("RefreshAllGridMoPai----------->")
    self.HNZZM_RePlayLogic:ForeachPlayerDatas(function(_,playerData) 
        local playerView = gameUIObjects.playerViews[self.HNZZM_RePlayLogic:GetCorrectUIIndex(self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(playerData.seat))];
...
function HNZZM_RePlayUILayer:AutoRegisterPlayerView(mySeat, size)
     for i = 1, self.HNZZM_RePlayLogic:GetMaxPlayerSize() do
        gameUIObjects.playerViews[i]:OpenView();
        self.HNZZM_RePlayLogic:RegisterPlayerView(gameUIObjects.playerViews[i]);
    end
    self.HNZZM_RePlayLogic:ResetAllPlayerView();
end

function HNZZM_RePlayUILayer:SetRoundStart( data )
    -- body
...
function HNZZM_RePlayUILayer:SetRoomInfo(roomData)
    self:SetDipaiNum(roomData.surplus);
    self:SetRoundNum(roomData.round);
end
function HNZZM_RePlayUILayer:SetGameRoundEnd( RoundEndData )
    -- body
    ...
    self.HNZZM_RePlayLogic:ResetGameState();
...
--清除一些UI上的状态
function HNZZM_RePlayUILayer:ClearUI()
    -- body
    --1.时钟
...
--刷新所有UI
function HNZZM_RePlayUILayer:RefreshAllUI()

end

function HNZZM_RePlayUILayer:RefreshPlayer()
    self.HNZZM_RePlayLogic:ForeachPlayerDatas(function(_,playerData)
        local playerView = gameUIObjects.playerViews[self.HNZZM_RePlayLogic:GetCorrectUIIndex(self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(playerData.seat))]; 
...
function HNZZM_RePlayUILayer:SetActivePlayer(roomData)
    ...
    local playerView = gameUIObjects.playerViews[self.HNZZM_RePlayLogic:GetCorrectUIIndex(self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(roomData.activeSeat))];
    playerView:SetActiveFlag(true and roomData.state == self.nmm_pb.GAMING);
end

function HNZZM_RePlayUILayer:GetOverTrusteeship()
    local Trusteeships={}
    for i = 0, self.roomData.setting.size-1 do
        local playerView = gameUIObjects.playerViews[self.HNZZM_RePlayLogic:GetCorrectUIIndex(self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(i))];
        Trusteeships[i] = playerView.trusteeshipFlag.gameObject.activeSelf;
    end
    return Trusteeships;
end

function HNZZM_RePlayUILayer:ChangePaiSize(hnzzmPaiSize)

end


function HNZZM_RePlayUILayer:SetGameRoundOver(RoundAllData)

    local overData = {};
    overData.RoundAllData   = RoundAllData;
    overData.roomInfo       = roomInfo;
    -- print("RoundAllData.data.complete:"..tostring(RoundAllData.data.complete)..",self.HNZZM_RePlayLogic.hasStageClear:"..tostring(self.HNZZM_RePlayLogic.hasStageClear));
    if not RoundAllData.data.complete and not self.HNZZM_RePlayLogic.hasStageClear then 
...
function HNZZM_RePlayUILayer.Update(obj)
    -- print("Update-------------------->");
   
end

--玩家摸牌
function HNZZM_RePlayUILayer:SetPlayerMoPai(moData)
    --先隐藏所有的摸牌的grid
    for i=1,#gameUIObjects.playerViews do
        gameUIObjects.playerViews[i].hnzzm_GridTool:SetGridMoVisible(false);
    end
    local playerView = gameUIObjects.playerViews[self.HNZZM_RePlayLogic:GetCorrectUIIndex(self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(moData.seat))];
    local playerData = self.HNZZM_RePlayLogic:GetPlayerDataBySeat(moData.seat);
    --显示当前玩家的摸牌的grid
    ...
    playerView.hnzzm_GridTool:GridHandPlatesDown(playerData.mahjongs,playerData.mahjongs);
    playerView.hnzzm_GridTool:RefreshMoPai(playerData.moMahjong);
    playerView.hnzzm_GridTool:MoPaiDown(playerData.moMahjong,playerData.mahjongCount);
   
    self:SetActivePlayer(self.roomData);
    if moData.operation and #moData.operation>0 then 
        for i=1,#moData.operation do
            if moData.operation[i].seat == self.HNZZM_RePlayLogic.mySeat then 
...
--玩家出牌
function HNZZM_RePlayUILayer:SetPlayerChuPai(chuData)
    -- print("player.seat"..chuData.seat.." chu pai,plate="..chuData.mahjong);
    local playerView    = gameUIObjects.playerViews[self.HNZZM_RePlayLogic:GetCorrectUIIndex(self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(chuData.seat))];
    local playerData    = self.HNZZM_RePlayLogic:GetPlayerDataBySeat(chuData.seat);
    local myData        = self.HNZZM_RePlayLogic:GetPlayerDataBySeat(self.HNZZM_RePlayLogic.mySeat);
    playerView.hnzzm_GridTool:RefreshGridThrow(playerData.paiHe);
    ...
    if chuData.seat == self.HNZZM_RePlayLogic.mySeat then 
        playerView.hnzzm_GridTool:SetGridMoVisible(false);
    end
    playerView.hnzzm_GridTool:RefreshGridHand(playerData.mahjongs,playerData.mahjongCount);
    playerView.hnzzm_GridTool:GridHandPlatesDown(playerData.mahjongs,playerData.mahjongs);
    ...
    if chuData.operation and #chuData.operation>0 then 
        for i=1,#chuData.operation do
            if chuData.operation[i].seat == self.HNZZM_RePlayLogic.mySeat then 
...
--玩家吃牌
function HNZZM_RePlayUILayer:SetPlayerChiPai(chiData)
    --1.显示特效
    local realIndex     = self.HNZZM_RePlayLogic:GetCorrectUIIndex(self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(chiData.seat));
    local outRealIndex  = self.HNZZM_RePlayLogic:GetCorrectUIIndex(self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(chiData.outSeat));
    local playerView    = gameUIObjects.playerViews[realIndex];
    local playerData    = self.HNZZM_RePlayLogic:GetPlayerDataBySeat(chiData.seat);
    local outPlayerData = self.HNZZM_RePlayLogic:GetPlayerDataBySeat(chiData.outSeat);
...
--玩家碰牌
function HNZZM_RePlayUILayer:SetPlayerPengPai(pengData)
    local realIndex     = self.HNZZM_RePlayLogic:GetCorrectUIIndex(self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(pengData.seat));
    local outRealIndex  = self.HNZZM_RePlayLogic:GetCorrectUIIndex(self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(pengData.outSeat));
    local playerView    = gameUIObjects.playerViews[realIndex];
    local playerData    = self.HNZZM_RePlayLogic:GetPlayerDataBySeat(pengData.seat);
    local outPlayerData = self.HNZZM_RePlayLogic:GetPlayerDataBySeat(pengData.outSeat);
    local outPlayerView = gameUIObjects.playerViews[outRealIndex];
    --显示特效
    ...
    if pengData.seat == self.HNZZM_RePlayLogic.mySeat then 
    ...
    if pengData.operation and #pengData.operation > 0 then 
        for i=1,#pengData.operation do
            if pengData.operation[i].seat == self.HNZZM_RePlayLogic.mySeat then 
...
--玩家杠牌
function HNZZM_RePlayUILayer:SetPlayerGangPai(gangData)
    --显示特效
    -- print("SetPlayerGangPai was called------------------------------");
    local realIndex     = self.HNZZM_RePlayLogic:GetCorrectUIIndex(self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(gangData.seat));
    local playerView    = gameUIObjects.playerViews[realIndex];
    local playerData    = self.HNZZM_RePlayLogic:GetPlayerDataBySeat(gangData.seat);
    ...
    if gangData.outSeat then 
        local outPlayerData = self.HNZZM_RePlayLogic:GetPlayerDataBySeat(gangData.outSeat);
        local outRealIndex  = self.HNZZM_RePlayLogic:GetCorrectUIIndex(self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(gangData.outSeat));
        local outPlayerView = gameUIObjects.playerViews[outRealIndex];
        outPlayerView.hnzzm_GridTool:RefreshGridThrow(outPlayerData.paiHe);
    end
    playerView.hnzzm_GridTool:SetGridMoVisible(false);
    ...
    if gangData.seat == self.HNZZM_RePlayLogic.mySeat then 
    ...
    if gangData.operation and #gangData.operation > 0 then 
        for i=1,#gangData.operation do
            if gangData.operation[i].seat == self.HNZZM_RePlayLogic.mySeat then 
...
--获取杠牌特效字符串
function HNZZM_RePlayUILayer:GetGangEffectString(gangType)
...
--玩家补牌
function HNZZM_RePlayUILayer:SetPlayerBuPai(buData)
    -- print("SetPlayerBuPai was called----------------------->");
end

--玩家过牌
function HNZZM_RePlayUILayer:SetPlayerGuoPai(guoData)
    -- print("SetPlayerGuoPai was called------------------------>");
    -- if guoData.seat == self.HNZZM_RePlayLogic.mySeat then 
...
--玩家胡牌
function HNZZM_RePlayUILayer:SetPlayerHuPai(huData)
    -- print("SetPlayerHuPai was called-------------------------->");
    local realIndex = self.HNZZM_RePlayLogic:GetCorrectUIIndex(self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(huData.seat));
    local playerView = gameUIObjects.playerViews[realIndex];
    local playerData = self.HNZZM_RePlayLogic:GetPlayerDataBySeat(huData.seat);
    ...
    -- print("huData.seat:"..huData.seat..",self.HNZZM_RePlayLogic.mySeat:"..self.HNZZM_RePlayLogic.mySeat);
    if huData.seat == self.HNZZM_RePlayLogic.mySeat then 
...
function HNZZM_RePlayUILayer:SetPao(winData)
    ...
    local realIndex = self.HNZZM_RePlayLogic:GetCorrectUIIndex(self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(winData.outSeat));
...
function HNZZM_RePlayUILayer:GetgameUIObjects()
    return gameUIObjects;
end


--根据背景设置文字颜色
function HNZZM_RePlayUILayer:SetGameColor(bgIndex)
...
function HNZZM_RePlayUILayer:getAccorColor(color)
	if color == 1 then
...
function HNZZM_RePlayUILayer:getNumColor(color)
	if color == 1 then
...
--选择背景
function HNZZM_RePlayUILayer:ChooseBG(bgIndex)
...
--设置东南西北旁边的局数数目
function HNZZM_RePlayUILayer:SetRoundNum(num)
...
--显示出牌的状态，东西南北，哪个该出牌了
--当前该出牌的玩家
function HNZZM_RePlayUILayer:ShowWaitOpreatEffect(activeSeat,show)
    ...
    for i=1,self.HNZZM_RePlayLogic.totalSize do
        local index = self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(activeSeat);
        self:SetESWNState(self.HNZZM_RePlayLogic:GetCorrectUIIndex(index),show);
    end
    --倒计时
    if show then 
...
function HNZZM_RePlayUILayer:SetESWNState(index,show)	gameUIObjects.ActivePlayerESWN[index]:Find('select').gameObject:SetActive(show);
...
function HNZZM_RePlayUILayer:ShowWaittingUI()
...
function HNZZM_RePlayUILayer:HideWaittingUI()
    -- print("HideWaittingUI---------------was called");
    -- print("NeedShowDiPai:"..tostring(HNZZM_PlayLogic.NeedShowDiPai(self.roomData,self.HNZZM_RePlayLogic)));
    gameUIObjects.roomSetting.gameObject:SetActive(true);
    gameUIObjects.roomSetting:Find("roominfo").gameObject:SetActive(false);
    gameUIObjects.dipai.gameObject:SetActive(HNZZM_PlayLogic.NeedShowDiPai(self.roomData,self.HNZZM_RePlayLogic));
    gameUIObjects.roomRound.gameObject:SetActive(HNZZM_PlayLogic.NeedShowDiPai(self.roomData,self.HNZZM_RePlayLogic));
...
function HNZZM_RePlayUILayer:getCardColor()
    return cardColor;
end

function HNZZM_RePlayUILayer:getcardText()
    return cardText;
end

function HNZZM_RePlayUILayer:GetAudioPrefix()
    return language == 1 and "" or "fy_";
end

----根据颜色值获取麻将背板图片
function HNZZM_RePlayUILayer:getColorCardName(name,colorIndex)
...
--根据用户选择选择麻将的牌面
function HNZZM_RePlayUILayer:GetCardTextName(plateIndex,cardTextIndex)
    return tostring(plateIndex);
end

--根据用户的选择选择牌面大小
function HNZZM_RePlayUILayer:GetCardPlateSize(plateIndex,cardTextIndex)
...
function HNZZM_RePlayUILayer:SetPlateColor(throwItemobj,showColor)
    if showColor then
...
--设置玩家出牌展示
function HNZZM_RePlayUILayer:SetPlayerChuPaiShow(seat,mahjong)
     local playerView = gameUIObjects.playerViews[self.HNZZM_RePlayLogic:GetCorrectUIIndex(self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(seat))];
     playerView.hnzzm_GridTool:RefreshChuPai(mahjong);
end

--设置箭头指向最后一个玩家出的牌
function HNZZM_RePlayUILayer:SetLastPlayerChuPaiEffect(seat)
    local playerView = gameUIObjects.playerViews[self.HNZZM_RePlayLogic:GetCorrectUIIndex(self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(seat))];
    local playerData = self.HNZZM_RePlayLogic:GetPlayerDataBySeat(seat);
...
--刷新玩家吃碰杠补等操作
function HNZZM_RePlayUILayer:RefreshOperationSend(operations,playerData)
...
--设置操作按钮的显示
function HNZZM_RePlayUILayer:SetOperationButtons(operations)
...
--玩家从操作列表中选择了操作
function HNZZM_RePlayUILayer:PlayerSelectOperation(operate)
...
function HNZZM_RePlayUILayer:ResetOperationTween( )
...
function HNZZM_RePlayUILayer:StartPlay()
    ...
    self.playCoroutine = coroutine.start(function() 
        while self.gameObject.activeSelf do 
            ...
            if not self.isPause then 
                ...
                self.HNZZM_RePlayLogic:OnPlayerPlay(record);
                ...
                if self.roomData.roundEnd and self.roomData.roundEnd~=nil then 
                    if self.playIndex > #self.roomData.records and #self.roomData.roundEnd.players > 0 then 
                        ...
                        coroutine.wait(0.5);
                        self.HNZZM_RePlayLogic:OnGameRoundEnd(self.roomData.roundEnd);
                        break;
                    end
                end
                coroutine.wait(self.playInterval);
...
--初始化播放控件
function HNZZM_RePlayUILayer:InitControls()
    self.isPause            = false;
    self.playInterval       = 1.5;
    self.playIndex          = 1;
    self.rawSpeed           = self.playInterval;
...
function HNZZM_RePlayUILayer:PlaySlow()
...
function HNZZM_RePlayUILayer:PlayFast()
...
function HNZZM_RePlayUILayer:PlayPause()
...
function HNZZM_RePlayUILayer:ExitPlay()
    coroutine.stop(self.playCoroutine);
    self.playCoroutine = nil;
    self.HNZZM_RePlayLogic:ForeachPlayerDatas(function(playerView,playerData) 
        playerView:CloseView(); 
        playerView.hnzzm_GridTool:HideAllGrid();
    end);
...
function HNZZM_RePlayUILayer:SetDismissType(go)
...
--设置玩家的抓鱼分数
function HNZZM_RePlayUILayer:SetPlayerFish(data)
    ...
    for i=1,#data.fishScoreBoard do
        local realIndex = self.HNZZM_RePlayLogic:GetCorrectUIIndex(self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(data.fishScoreBoard[i].seat));
        local playerView = gameUIObjects.playerViews[realIndex];
        local playerData = self.HNZZM_RePlayLogic:GetPlayerDataBySeat(data.fishScoreBoard[i].seat);
...
--设置玩家分数
function HNZZM_RePlayUILayer:SetPlayerScore(data)
    ...
    for i=1,#data.scoreBoard do
        local playerData    = self.HNZZM_RePlayLogic:GetPlayerDataBySeat(data.scoreBoard[i].seat);
        local realIndex     = self.HNZZM_RePlayLogic:GetCorrectUIIndex(self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(data.scoreBoard[i].seat));
        local playerView    = gameUIObjects.playerViews[realIndex];
...
function HNZZM_RePlayUILayer:SetPlayerHunPlates(hunPlates)
    self:HunPlatesAnimation(hunPlates);
end


--播放混牌的动画
function HNZZM_RePlayUILayer:HunPlatesAnimation(hunPlates)
...
--设置展示的混牌
function HNZZM_RePlayUILayer:SetHunPlates(hunPlates)
    local hunGrid = gameUIObjects.hunPanel:Find("hunGrid");
    hunGrid.gameObject:SetActive(true);
...
--获取游戏中的混牌
function HNZZM_RePlayUILayer:GetHunPlates()
    return self.HNZZM_RePlayLogic:GetPlayerDataBySeat(self.HNZZM_RePlayLogic.mySeat).hunMahjongs;
end


function HNZZM_RePlayUILayer:SetPlayerSelectPao()
    self.HNZZM_RePlayLogic:ForeachPlayerDatas(function(_,playerData) 
        local realIndex = self.HNZZM_RePlayLogic:GetCorrectUIIndex(self.HNZZM_RePlayLogic:GetPlayerViewIndexBySeat(playerData.seat));
        local playerView = gameUIObjects.playerViews[realIndex];
