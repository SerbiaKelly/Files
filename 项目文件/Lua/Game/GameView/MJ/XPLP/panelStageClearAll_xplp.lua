...
panelStageClearAll_xplp = {}
local this = panelStageClearAll_xplp
...
--	四人
	...
	for i = 1, 4 do
		...
		playerSize4[i].GuChouCount 		= playerSize4[i].transform:Find("ScoreGroup/grid/GuChouCount");
...
--	三人和二人
	for i = 1, 3 do
		...
		playerSize3and2[i].GuChouCount 		= playerSize3and2[i].transform:Find("ScoreGroup/grid/GuChouCount");
...
function this.OnClickMask(go)
	...
	PanelManager.Instance:HideWindow('panelInGame_xplp');
...
function this.OnClickMatchNext(go)
	AudioManager.Instance:PlayAudio('btn')
	print("panelInGame.HideCoroutine is nil:"..tostring(panelInGame.HideCoroutine == nil));
...
function this.SetPlayerInfo(playerObj,playerData)
	...
	--12.箍臭
	playerObj.GuChouCount:GetComponent("UILabel").text 		= "箍臭次数："..playerData.guChou;
	--总得分
	playerObj.totalSocre:GetComponent("UILabel").text 		= playerData.score;
	...
	if playerData.fee ~= nil then
		...
		sumGetPos.y = tonumber(playerData.fee)~=0 and -185 or -198.9
		playerObj.ScoreGet.localPosition = sumGetPos
		local sumLosePos = playerObj.ScoreLose.localPosition
		sumLosePos.y = tonumber(playerData.fee)~=0 and -185 or -198.9
