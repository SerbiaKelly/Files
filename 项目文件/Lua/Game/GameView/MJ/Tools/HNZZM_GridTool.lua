...
require("HNZZM_Tools");
HNZZM_GridTool  = {};
HNZZM_GridTool  = class();
local this      = HNZZM_GridTool;
local uiLayer   = nil;
...
function HNZZM_GridTool:ctor(gridHand,gridXi,gridThrow,gridMo,chuPaiShow,posIndex,callBacktable,extraParameter)
...
--刷新手牌grid，填充数据
function HNZZM_GridTool:RefreshGridHand(plates,mahjongCount)
    -- body
    if plates == nil then 
        return ;
    end
            ...
            if self.posIndex == 0 then 
                ...
                for j=1,#hunPlates do
                    if plates[i+1] == hunPlates[j] then 
                        cardItem:Find("mark").gameObject:SetActive(uiLayer.roomData.setting.takeHun);
                        cardItem:Find("mark/marklabel"):GetComponent("UILabel").text = "混";
                    else
...
function this.GetGridMoPos(gridHand,gridMo, posIndex,mahjongCount)
    ...
    if posIndex == 0 then 
        scale = HNZZM_Tools.IfNeedAutoLayOut() and Vector3(FitScale, FitScale, 1) or Vector3(1.0,1.0,1.0);
    end
	if posIndex == 0 then
		pos.y = gridHand.localPosition.y;
		pos.x = HNZZM_Tools.IfNeedAutoLayOut() and 574*FitScale or gridMo.localPosition.x;
...
--刷新吃碰杠补,填充数据
function HNZZM_GridTool:RefreshGridXi(operationCards)
    ...
    if self.posIndex == 0 then 
        self.gridXi.localScale = HNZZM_Tools.IfNeedAutoLayOut() and Vector3(FitScale, FitScale, 1) or Vector3(1.0,1.0,1.0);
        self.gridXi.localPosition = HNZZM_Tools.IfNeedAutoLayOut() and Vector3(-636-FitOffset, -315, 0) or Vector3(-638,-314.9,0);
    elseif self.posIndex ==1 then 
        self.gridXi.localPosition = HNZZM_Tools.IfNeedAutoLayOut() and Vector3(594,-160,0) or Vector3(526,-160.9,0);
    elseif self.posIndex ==3 then 
        self.gridXi.localPosition = HNZZM_Tools.IfNeedAutoLayOut() and Vector3(-595,287,0) or Vector3(-529.6,287,0);
    end
...
function this.GetGridHandPos(gridXi,gridHand,posIndex,xiGridData)
    ...
    elseif posIndex == 1 then 
        if #xiGridData > 0 then 
            ...
            pos.x = HNZZM_Tools.IfNeedAutoLayOut() and 607.3 or gameUIObjects.playerHandGridPos[posIndex+1].x;
        else
            pos = HNZZM_Tools.IfNeedAutoLayOut() and Vector3(607.3,-146,0) or  gameUIObjects.playerHandGridPos[posIndex+1];
        end
    elseif posIndex == 2 then 
        if #xiGridData > 0 then 
        ...
        if #xiGridData > 0 then 
            ...
            pos.x = HNZZM_Tools.IfNeedAutoLayOut() and -610 or gameUIObjects.playerHandGridPos[posIndex+1].x;
        else
            pos = HNZZM_Tools.IfNeedAutoLayOut() and Vector3(-610,265,0) or  gameUIObjects.playerHandGridPos[posIndex+1];
        end
    end
...
--刷新打出去的牌，填充数据
function HNZZM_GridTool:RefreshGridThrow(plates)
    if not plates then 
        return ;
    end
...
--将已出的牌盖起来
function HNZZM_GridTool:CoverGridThrow()
...
--刷新摸的牌，填充数据
function HNZZM_GridTool:RefreshMoPai(plate)
        ...
        for i=1,#hunPlates do
            if cardMark ~= nil then 
                ...
                if plate == hunPlates[i] then 
                    print("uiLayer.roomData.setting.takeHun="..tostring(uiLayer.roomData.setting.takeHun))
                    cardMark.gameObject:SetActive(uiLayer.roomData.setting.takeHun);
...
--摸牌的倒牌
function HNZZM_GridTool:MoPaiDown(plate,gridMahjongCount)
...
--手牌倒牌
function HNZZM_GridTool:GridHandPlatesDown(plates,downPlates)
    if plates == nil then 
        return ;
    end
    if self.posIndex == 0 then 
...
function HNZZM_GridTool:RefreshChuPai(plate)
...
--盖住出牌
function HNZZM_GridTool:CoverChupai()
    self.chuPaiShow.gameObject:SetActive(true);
    local cardItemBG            = self.chuPaiShow:GetComponent("UISprite");
    local cardItemPlate         = self.chuPaiShow:Find("card"):GetComponent("UISprite");
...
function HNZZM_GridTool:SetGridHandVisible(enable)
    self.gridHand.gameObject:SetActive(enable);
end

function HNZZM_GridTool:SetGridThrowVisible(enable)
    self.gridThrow.gameObject:SetActive(enable);
end

function HNZZM_GridTool:SetGridMoVisible(enable)
    self.gridMo.gameObject:SetActive(enable);
end

function HNZZM_GridTool:SetGridXiVisible(enable)
    self.gridXi.gameObject:SetActive(enable);
end

function HNZZM_GridTool:HideAllGrid()
...
function HNZZM_GridTool:ShowAllGrid()
...
function HNZZM_GridTool:SetLayOut()
...
function this.SetHandGridLayOut(gridHand,posIndex,playerSize)
    ...
    -- print("HNZZM_Tools.IfNeedAutoLayOut()="..tostring(HNZZM_Tools.IfNeedAutoLayOut()));
    local gameUIObjects = uiLayer:GetgameUIObjects();
    if playerSize == 2 then --两个人的排版
        if posIndex == 0 then 
            gridHand.localScale = HNZZM_Tools.IfNeedAutoLayOut() and  Vector3(FitScale, FitScale, 1) or Vector3(1.0,1.0,1.0);
            gridHand.localPosition = HNZZM_Tools.IfNeedAutoLayOut() and Vector3(455*FitScale, -288, 0) or gameUIObjects.playerHandGridPos[posIndex+1];
        end
    elseif playerSize == 3 then --三个人的排版
        if posIndex == 0 then 
            gridHand.localScale = HNZZM_Tools.IfNeedAutoLayOut() and Vector3(FitScale, FitScale, 1) or Vector3(1.0,1.0,1.0);
            gridHand.localPosition = HNZZM_Tools.IfNeedAutoLayOut() and Vector3(455*FitScale, -288, 0) or gameUIObjects.playerHandGridPos[posIndex+1];
        elseif posIndex == 1 then 
            gridHand.localPosition = HNZZM_Tools.IfNeedAutoLayOut() and  Vector3(542,-182.6,0) or Vector3(542,-182.6,0) ;
        elseif posIndex == 3 then
            gridHand.localPosition = HNZZM_Tools.IfNeedAutoLayOut() and Vector3(-654,242.8,0) or Vector3(-545,242.8,0) ;
        end
    else--四个人的排版
        if posIndex == 0 then 
            gridHand.localScale = HNZZM_Tools.IfNeedAutoLayOut() and Vector3(FitScale, FitScale, 1) or Vector3(1.0,1.0,1.0);
        ...
        elseif posIndex == 1 then 
            gridHand.localPosition = HNZZM_Tools.IfNeedAutoLayOut() and  Vector3(542,-182.6,0) or Vector3(542,-182.6,0) ;
        elseif posIndex == 3 then 
            gridHand.localPosition = HNZZM_Tools.IfNeedAutoLayOut() and Vector3(-545,242.8,0) or Vector3(-545,242.8,0) ;
        else
