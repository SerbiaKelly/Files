...
HNHSM_GridTool  = class();
local this      = HNHSM_GridTool;
local uiLayer   = nil;
...
function HNHSM_GridTool:ctor(gridHand,gridXi,gridThrow,gridMo,chuPaiShow,posIndex,callBacktable,extraParameter)
...
--刷新手牌grid，填充数据
function HNHSM_GridTool:RefreshGridHand(plates,mahjongCount)
    -- body
    if plates == nil then 
        return ;
    end
    if self.posIndex == 0 then 
        ...
        --处理红中
        if uiLayer.roomData.setting.variableHongZhong then
            local hongzhongCount = GetMJPlateCount(plates,uiLayer.HongzhongPlateValue);--红中的值是31
            tableRemoveByKeys(plates,getRemoveKeys({uiLayer.HongzhongPlateValue}));
            for i = 1, hongzhongCount do
                tableAdd(plates,{uiLayer.HongzhongPlateValue});
            end
        end
    else
    ...
    for i=0,self.gridHand.childCount -1 do
        if i < mahjongCount then 
            ...
            if self.posIndex == 0 then 
                ...
                --赖子检测
                if plates[i+1] == uiLayer.HongzhongPlateValue then 
                    cardItem:Find("mark").gameObject:SetActive(uiLayer.roomData.setting.variableHongZhong);
                    cardItem:Find("mark/marklabel"):GetComponent("UILabel").text = "赖";
                else
...
function this.GetGridMoPos(gridHand,gridMo, posIndex,mahjongCount)
    ...
    if posIndex == 0 then 
        scale = HNHSM_Tools.IfNeedAutoLayOut() and Vector3(FitScale, FitScale, 1) or Vector3(1.0,1.0,1.0);
    end
	if posIndex == 0 then
		pos.y = gridHand.localPosition.y;
		pos.x = HNHSM_Tools.IfNeedAutoLayOut() and 574*FitScale or gridMo.localPosition.x;
...
--刷新吃碰杠补,填充数据
function HNHSM_GridTool:RefreshGridXi(operationCards)
    ...
    if self.posIndex == 0 then 
        self.gridXi.localScale = HNHSM_Tools.IfNeedAutoLayOut() and Vector3(FitScale, FitScale, 1) or Vector3(1.0,1.0,1.0);
        self.gridXi.localPosition = HNHSM_Tools.IfNeedAutoLayOut() and Vector3(-636-FitOffset, -315, 0) or Vector3(-638,-314.9,0);
    elseif self.posIndex ==1 then 
        self.gridXi.localPosition = HNHSM_Tools.IfNeedAutoLayOut() and Vector3(594,-160,0) or Vector3(526,-160.9,0);
    elseif self.posIndex ==3 then 
        self.gridXi.localPosition = HNHSM_Tools.IfNeedAutoLayOut() and Vector3(-595,287,0) or Vector3(-529.6,287,0);
    end
...
function this.GetGridHandPos(gridXi,gridHand,posIndex,xiGridData)
    ...
    elseif posIndex == 1 then 
        if #xiGridData > 0 then 
            ...
            pos.x = HNHSM_Tools.IfNeedAutoLayOut() and 607.3 or gameUIObjects.playerHandGridPos[posIndex+1].x;
        else
            pos = HNHSM_Tools.IfNeedAutoLayOut() and Vector3(607.3,-146,0) or  gameUIObjects.playerHandGridPos[posIndex+1];
        end
    elseif posIndex == 2 then 
        if #xiGridData > 0 then 
        ...
        if #xiGridData > 0 then 
            ...
            pos.x = HNHSM_Tools.IfNeedAutoLayOut() and -610 or gameUIObjects.playerHandGridPos[posIndex+1].x;
        else
            pos = HNHSM_Tools.IfNeedAutoLayOut() and Vector3(-610,265,0) or  gameUIObjects.playerHandGridPos[posIndex+1];
        end
    end
...
--刷新打出去的牌，填充数据
function HNHSM_GridTool:RefreshGridThrow(plates)
    if not plates then 
        return ;
    end
...
--将已出的牌盖起来
function HNHSM_GridTool:CoverGridThrow()
...
--刷新摸的牌，填充数据
function HNHSM_GridTool:RefreshMoPai(plate)
    ...
    if self.posIndex == 0 then 
        ...
        if cardMark ~= nil then 
            if plate == uiLayer.HongzhongPlateValue then 
                cardMark.gameObject:SetActive(uiLayer.roomData.setting.variableHongZhong);
...
--摸牌的倒牌
function HNHSM_GridTool:MoPaiDown(plate,gridMahjongCount)
...
--手牌倒牌
function HNHSM_GridTool:GridHandPlatesDown(plates,downPlates)
    if plates == nil then 
        return ;
    end
    if self.posIndex == 0 then 
    ...
    for i=1,#plates do
        ...
        for j=#downPlates,1,-1 do
            if downPlates[j] == plates[i] then 
                ...
                local cardMark = cardItemBG.transform:Find("mark");
                if cardMark ~= nil then 
                    if downPlates[j] == uiLayer.HongzhongPlateValue then 
                        cardMark.gameObject:SetActive(uiLayer.roomData.setting.variableHongZhong);
                        cardMark:Find("marklabel"):GetComponent("UILabel").text = "赖";
                    else
                        cardMark.gameObject:SetActive(false);
...
function HNHSM_GridTool:RefreshChuPai(plate)
...
--盖住出牌
function HNHSM_GridTool:CoverChupai()
    self.chuPaiShow.gameObject:SetActive(true);
    local cardItemBG            = self.chuPaiShow:GetComponent("UISprite");
    local cardItemPlate         = self.chuPaiShow:Find("card"):GetComponent("UISprite");
...
function HNHSM_GridTool:SetGridHandVisible(enable)
    self.gridHand.gameObject:SetActive(enable);
end

function HNHSM_GridTool:SetGridThrowVisible(enable)
    self.gridThrow.gameObject:SetActive(enable);
end

function HNHSM_GridTool:SetGridMoVisible(enable)
    self.gridMo.gameObject:SetActive(enable);
end

function HNHSM_GridTool:SetGridXiVisible(enable)
    self.gridXi.gameObject:SetActive(enable);
end

function HNHSM_GridTool:HideAllGrid()
...
function HNHSM_GridTool:ShowAllGrid()
...
function this.GetXiGridData(operationCards)
    ...
    for i=1,#operationCards do
        ...
        if operation_type == hnm_pb.SORT_CHI then 
            table.insert( xiGridData, {operation == hnm_pb.CHI,plates = {op_cards[1] , op_cards[2], op_cards[3]}} );
        elseif operation_type == hnm_pb.SORT_PENG then 
            table.insert(xiGridData, {operation = hnm_pb.PENG, plates={op_cards[1] , op_cards[1], op_cards[1]}});
        elseif operation_type == hnm_pb.SORT_MING then
			table.insert(xiGridData, {operation = hnm_pb.GANG, plates={op_cards[1], op_cards[1], op_cards[1], op_cards[1]}, state = hnm_pb.MING});
		elseif operation_type == hnm_pb.SORT_DARK then
			table.insert(xiGridData, {operation = hnm_pb.GANG, plates={op_cards[1], op_cards[1], op_cards[1], op_cards[1]}, state = hnm_pb.AN});
		elseif operation_type == hnm_pb.SORT_DIAN then
			table.insert(xiGridData, {operation = hnm_pb.GANG, plates={op_cards[1], op_cards[1], op_cards[1], op_cards[1]}, state = hnm_pb.DIAN});
        end
    end
...
function HNHSM_GridTool:SetLayOut()
...
function this.SetHandGridLayOut(gridHand,posIndex,playerSize)
    ...
    -- print("HNHSM_Tools.IfNeedAutoLayOut()="..tostring(HNHSM_Tools.IfNeedAutoLayOut()));
    local gameUIObjects = uiLayer:GetgameUIObjects();
    if playerSize == 2 then --两个人的排版
        if posIndex == 0 then 
            gridHand.localScale = HNHSM_Tools.IfNeedAutoLayOut() and  Vector3(FitScale, FitScale, 1) or Vector3(1.0,1.0,1.0);
            gridHand.localPosition = HNHSM_Tools.IfNeedAutoLayOut() and Vector3(455*FitScale, -288, 0) or gameUIObjects.playerHandGridPos[posIndex+1];
        end
    elseif playerSize == 3 then --三个人的排版
        if posIndex == 0 then 
            gridHand.localScale = HNHSM_Tools.IfNeedAutoLayOut() and Vector3(FitScale, FitScale, 1) or Vector3(1.0,1.0,1.0);
            gridHand.localPosition = HNHSM_Tools.IfNeedAutoLayOut() and Vector3(455*FitScale, -288, 0) or gameUIObjects.playerHandGridPos[posIndex+1];
        elseif posIndex == 1 then 
            gridHand.localPosition = HNHSM_Tools.IfNeedAutoLayOut() and  Vector3(542,-182.6,0) or Vector3(542,-182.6,0) ;
        elseif posIndex == 3 then
            gridHand.localPosition = HNHSM_Tools.IfNeedAutoLayOut() and Vector3(-654,242.8,0) or Vector3(-545,242.8,0) ;
        end
    else--四个人的排版
        if posIndex == 0 then 
            gridHand.localScale = HNHSM_Tools.IfNeedAutoLayOut() and Vector3(FitScale, FitScale, 1) or Vector3(1.0,1.0,1.0);
        ...
        elseif posIndex == 1 then 
            gridHand.localPosition = HNHSM_Tools.IfNeedAutoLayOut() and  Vector3(542,-182.6,0) or Vector3(542,-182.6,0) ;
        elseif posIndex == 3 then 
            gridHand.localPosition = HNHSM_Tools.IfNeedAutoLayOut() and Vector3(-545,242.8,0) or Vector3(-545,242.8,0) ;
        else
