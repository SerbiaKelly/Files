...
function HNHSM_UILayer:ctor(gameObject,HNHSM_Logic,game_pb)
    this                = self;
    self.HNHSM_Logic    = HNHSM_Logic;
    self.hnm_pb         = game_pb;
    self.gameObject     = gameObject;
    self:GetUIS(self.gameObject);
    local inGameUpdate  = panelInGame_hnhsm.Update;
    panelInGame_hnhsm.Update = function(obj) 
        inGameUpdate(obj);
        this.Update(obj);
    end ;
...
function HNHSM_UILayer:GetGameLogic()
    return self.HNHSM_Logic;
end


function HNHSM_UILayer:GetTrusteeShipStates()
    return TrusteeShipStates;
end


function HNHSM_UILayer:GetUIS(gameObject)
...
function HNHSM_UILayer:GetPlayersUI(gameObject)
    local maxPlayerSize = self.HNHSM_Logic:GetMaxPlayerSize();
    ...
    for i=1,maxPlayerSize do
        ...
        local hnhsm_GridTool           = HNHSM_GridTool.New(gridHand,gridXi,gridThrow,gridMo,chuPaiShow,(i-1),callBacktable,{uiLayer = self});
        local playerView            = HNHSM_Player.New(obj.gameObject,gameObject);
        local RoundDetail           = {};
        RoundDetail.openUserCard    = true;
        RoundDetail.isLord          = true;
        playerView:SetGameLogic(self.HNHSM_Logic):SetMJGridTool(hnhsm_GridTool,self.HNHSM_Logic,self);
        playerView:SetRoundDetail(RoundDetail);
        hnhsm_GridTool:HideAllGrid();
...
function HNHSM_UILayer:GetPlatesCallBacks(index)
...
function HNHSM_UILayer:GetGPSUI(gameObject)
...
function HNHSM_UILayer:GetButtons(gameObject)
...
function HNHSM_UILayer:GetRoomInfoUI(gameObject)
...
function HNHSM_UILayer:InitUILayer(roomData)
    ...
    HNHSM_EventBind.Init(self,gameUIObjects,self.HNHSM_Logic);--绑定事件

end

function HNHSM_UILayer:InitRoomInfo(roomData)
    --获取主题颜色
	...
	outPlateType 	= UnityEngine.PlayerPrefs.GetInt('outPlateType_hnhsm', 0);
    coroutine.start(RefreshTime,gameUIObjects.roomTime:GetComponent("UILabel"),60);
    coroutine.start(RefreshPhoneState,self.gameObject,self.HNHSM_Logic,gameUIObjects.batteryLevel,gameUIObjects.network);
    ...
    gameUIObjects.roomSetting:Find('Label'):GetComponent('UILabel').text    = self.roomData.clubId ~= "0" and GetHNHSMRuleText(roomData.setting,false,false) or GetHNHSMRuleText(roomData.setting,false,true);
    ...
    if self.roomData.state == self.hnm_pb.GAMING then --已在游戏中了，那肯定时隐藏的
        ...
        if self.roomData.round > 1 then --不是第一局了，肯定也是隐藏的
            ...
            self:SetInviteVisiable(self.HNHSM_Logic:GetPlayerDataLength() ~= self.roomData.setting.size or 
            ((not self.HNHSM_Logic:GetPlayerDataBySeat(self.HNHSM_Logic.mySeat).ready) or (not self.HNHSM_Logic:IsAllReaded()) ));
    ...
    --游戏开始时是否显示蹲拉跑
    local myData = self.HNHSM_Logic:GetPlayerDataBySeat(self.HNHSM_Logic.mySeat);
    if myData and myData.completeChooce == 1 then 
        PanelManager.Instance:ShowWindow("panelBetOn",{seat = self.HNHSM_Logic.mySeat,isBanker = self.HNHSM_Logic:IsBanker()})
    end
...
function HNHSM_UILayer:SetRoomRestTime()
    if self.roomData.state == self.hnm_pb.READYING and self.roomData.clubId ~= "0" and self.roomData.round == 1 then
        ...
        self.roomRestTimeCor =  HNHSM_Tools.RefreshRoomRestTime(self.roomData.time/1000,
        function(time)
...
function HNHSM_UILayer:InitPlayerInfo(roomData)
    ...
    DestroyRoomData.playerData      = self.HNHSM_Logic:GetPlayerDatas();
    DestroyRoomData.mySeat 		    = self.HNHSM_Logic.mySeat;
    local dissolution               = self.HNHSM_Logic:GetDissolutions();
    ...
    --设置托管
    self:SetPlayerTrustesship(self.HNHSM_Logic.mySeat,self.HNHSM_Logic:GetPlayerDataBySeat(self.HNHSM_Logic.mySeat).trusteeship);
...
function HNHSM_UILayer:InitAllGrids(roomData)
    --隐藏所有的grid
    ...
    --处理听牌
    local myData        = self.HNHSM_Logic:GetPlayerDataBySeat(self.HNHSM_Logic.mySeat);
    local myRealIndex   = self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(myData.seat));
    ...
    if #myData.tingTip>0 then 
        self:SetTiPlateMark(myPlayerView.hnhsm_GridTool.gridHand,myData.mahjongs,myData.tingTip,myData);
        self:SetTiPlateMark(myPlayerView.hnhsm_GridTool.gridMo,{myData.moMahjong},myData.tingTip,myData);
        
    end
end

function HNHSM_UILayer:InitGPSInfo(roomData)
    gameUIObjects.GpsView.gameObject:SetActive(false);
end

function HNHSM_UILayer:InitESWNInfo(roomData)
    gameUIObjects.dipai.gameObject:SetActive(false);
    self:SetDipaiNum(roomData.surplus);
    self:SetRoundNum(roomData.round);
    gameUIObjects.dipai.gameObject:SetActive(HNHSM_PlayLogic.NeedShowDiPai(self.roomData,self.HNHSM_Logic));
    gameUIObjects.roomRound.gameObject:SetActive(HNHSM_PlayLogic.NeedShowDiPai(self.roomData,self.HNHSM_Logic));
end


function HNHSM_UILayer:SetPlayerPos(roomData)
    --设置座位位置
    for i=1,#gameUIObjects.playerViews do
        local playerView =  gameUIObjects.playerViews[i];
        playerView:SetPos(roomData.state  ~= self.hnm_pb.GAMING and (self.HNHSM_Logic.totalSize < roomData.setting.size or roomData.round == 1) );
        playerView:NeedHideSelf(roomData.setting.size);
    end

    --设置语音，聊天，还有听牌按钮
    gameUIObjects.ButtonSound.localPosition                     = HNHSM_Tools.IfNeedAutoLayOut() and Vector3(700,55,0) or Vector3(617,55,0);
    gameUIObjects.ButtonChat.localPosition                      = HNHSM_Tools.IfNeedAutoLayOut() and Vector3(700,-41.9,0) or Vector3(617,-55,0);
    gameUIObjects.playerTingButton.localPosition                = HNHSM_Tools.IfNeedAutoLayOut() and Vector3(709.8,-153.2,0) or Vector3(625,-178,0);
    gameUIObjects.newTingOperation.localPosition                = HNHSM_Tools.IfNeedAutoLayOut() and Vector3(-363,-177,0) or Vector3(-363,-200,0);
    gameUIObjects.playerTingButton:Find("frame").localPosition  = HNHSM_Tools.IfNeedAutoLayOut() and Vector3(-724.8,64,0) or Vector3(-642,64,0);

    
	
end

function HNHSM_UILayer:InitTingButton()
    local myData = self.HNHSM_Logic:GetPlayerDataBySeat(self.HNHSM_Logic.mySeat);
...
--刷新所有grid
function HNHSM_UILayer:RefreshAllGrid( )
    -- body
    -- print("RefreshAllGrid------------->")
    
    self.HNHSM_Logic:ForeachPlayerDatas(function(playerView,_) 
...
--刷新手牌
function HNHSM_UILayer:RefreshAllGridHand()
    -- print("RefreshAllGridHand----------->")
    self.HNHSM_Logic:ForeachPlayerDatas(function(playerView,playerData) 
...
--刷新喜牌
function HNHSM_UILayer:RefreshAllGridXi()
...
--刷新带出去的牌
function HNHSM_UILayer:RefreshAllGridThrow()
...
--刷新摸牌
function HNHSM_UILayer:RefreshAllGridMoPai()
    -- print("RefreshAllGridMoPai----------->")
    self.HNHSM_Logic:ForeachPlayerDatas(function(_,playerData) 
        local playerView = gameUIObjects.playerViews[self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(playerData.seat))];
...
--盖住所有出的牌
function HNHSM_UILayer:CoverAllGridThrow()
    -- print("CoverAllGridThrow was called-------------->");
    self.HNHSM_Logic:ForeachPlayerDatas(function(_,playerData) 
        local playerView = gameUIObjects.playerViews[self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(playerData.seat))];
        playerView.hnhsm_GridTool:CoverGridThrow();
    end);
end

--取消盖住所有出的牌
function HNHSM_UILayer:UnCoverAllGridThrow()
    -- print("UnCoverAllGridThrow was called-------------->");
    self:RefreshAllGridThrow();
end

function HNHSM_UILayer:AutoRegisterPlayerView(mySeat, size)
    for i = 1, self.HNHSM_Logic:GetMaxPlayerSize() do
        gameUIObjects.playerViews[i]:OpenView();
        self.HNHSM_Logic:RegisterPlayerView(gameUIObjects.playerViews[i]);
    end
    self.HNHSM_Logic:ResetAllPlayerView();
end

function HNHSM_UILayer:SetRoundStart( data )
    -- body
...
function HNHSM_UILayer:SetRoomInfo(roomData)
    self:SetDipaiNum(roomData.surplus);
    self:SetRoundNum(roomData.round);

end
function HNHSM_UILayer:SetGameRoundEnd( RoundEndData )
    -- body
    ...
    self.HNHSM_Logic:ResetGameState();
...
--清除一些UI上的状态
function HNHSM_UILayer:ClearUI( ... )
    -- body
    --1.时钟
    ...
    --3.提牌标志
    local myPlayerView = gameUIObjects.playerViews[self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(self.HNHSM_Logic.mySeat))];
    self:HideTiMark(myPlayerView.hnhsm_GridTool.gridHand);
    self:HideTiMark(myPlayerView.hnhsm_GridTool.gridMo);
    --隐藏玩家的听牌按钮
    gameUIObjects.playerTingButton.gameObject:SetActive(false);
    ...
    --8.抓鱼分数清理
    self.HNHSM_Logic:ForeachPlayerDatas(function(_,playerData) 
        local realIndex = self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(playerData.seat));
...
--刷新所有UI
function HNHSM_UILayer:RefreshAllUI()

end

--玩家再游戏期间申请解散
function HNHSM_UILayer:SetDissolve(dissolveData)
    DestroyRoomData.playerData 	= self.HNHSM_Logic:GetPlayerDatas();
...
function HNHSM_UILayer:SetDestoryPanelShow()
    PanelManager.Instance:HideWindow('panelDestroyRoom');
...
function HNHSM_UILayer:RefreshGpsView()
    ...
    if self.gpsTool == nil then 
        self.gpsTool = GPSTool.New(self.HNHSM_Logic,self.roomData,gameUIObjects.distances,gameUIObjects.gpsPlayers);
    end
    self.gpsTool:Refresh(self.HNHSM_Logic,self.roomData);
end

function HNHSM_UILayer:RefreshPlayer()
    self.HNHSM_Logic:ForeachPlayerDatas(function(_,playerData)
        local playerView = gameUIObjects.playerViews[self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(playerData.seat))]; 
        -- print("self.roomData.state:"..self.roomData.state);
    ...
    --如果自己准备好了，就隐藏准备按钮
    gameUIObjects.ButtonReady.gameObject:SetActive(not self.HNHSM_Logic:GetPlayerDataBySeat(self.HNHSM_Logic.mySeat).ready);
     --设置托管
    self:SetPlayerTrustesship(self.HNHSM_Logic.mySeat,self.HNHSM_Logic:GetPlayerDataBySeat(self.HNHSM_Logic.mySeat).trusteeship);
    self:NotifyGameStart();
    if self.HNHSM_Logic:IsAllReaded() then 
        self:SetInviteVisiable(false);
    end
     --如果所有人都准备了，那么要隐藏解散房间倒计时
    if self.roomData.clubId ~= 0 and self.HNHSM_Logic:IsAllReaded() then 
        if self.roomRestTimeCor ~= nil then 
            coroutine.stop(self.roomRestTimeCor);
            self.roomRestTimeCor = nil;
        end
...
--有玩家加入
function HNHSM_UILayer:OnPlayerJoin(data)
    self:NotifyGameStart();
end



function HNHSM_UILayer:SetActivePlayer(roomData)
    ...
    local playerView = gameUIObjects.playerViews[self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(roomData.activeSeat))];
    playerView:SetActiveFlag(true and roomData.state == self.hnm_pb.GAMING);
end

function HNHSM_UILayer:GetOverTrusteeship()
    local Trusteeships={}
    for i = 0, self.roomData.setting.size-1 do
        local playerView = gameUIObjects.playerViews[self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(i))];
        Trusteeships[i] = playerView.trusteeshipFlag.gameObject.activeSelf;
    end
    return Trusteeships;
end

function HNHSM_UILayer:ChangePaiSize(hnhsmPaiSize)

end


function HNHSM_UILayer:SetGameRoundOver(RoundAllData)

    local overData = {};
    overData.RoundAllData   = RoundAllData;
    overData.roomInfo       = roomInfo;
    -- print("RoundAllData.data.complete:"..tostring(RoundAllData.data.complete)..",self.HNHSM_Logic.hasStageClear:"..tostring(self.HNHSM_Logic.hasStageClear));
    if not RoundAllData.data.complete and not self.HNHSM_Logic.hasStageClear then 
...
function HNHSM_UILayer:SetGameDestroy(roomData)
    -- 清理UI
    self:ClearUI();
    panelInGame.fanHuiRoomNumber = nil;
    --隐藏打骰子
    if panelStageClear_hnhsm then 
        panelStageClear_hnhsm.setButtonsStatus(true);
    end
    -- 返回大厅或者俱乐部
    panelInGame.HideCoroutine = StartCoroutine(function()
        ...
        if roomData.clubId ~= '0' then
            PanelManager.Instance:ShowWindow('panelClub', {name = 'panelInGame_hnhsm'});
        else
...
function HNHSM_UILayer.Update(obj)
    -- print("Update-------------------->");
...
function HNHSM_UILayer.OnDragStartPlate(obj)
...
function HNHSM_UILayer.OnDragEndtPlate(obj)
    ...
    local myData     = this.HNHSM_Logic:GetPlayerDataBySeat(this.HNHSM_Logic.mySeat);
    local playerView = gameUIObjects.playerViews[this.HNHSM_Logic:GetCorrectUIIndex(this.HNHSM_Logic:GetPlayerViewIndexBySeat(myData.seat))];
    --能出牌的条件，1.轮到自己出牌，2.没有封牌，3.在线状态,4.牌拖到了出牌位置
    if this.HNHSM_Logic:CanChuPai() and this.HNHSM_Logic.connect.IsConnect and (not this.HNHSM_Logic:GetPlayerDataBySeat(this.HNHSM_Logic.mySeat).sealPai ) then 
        ...
        if UICamera.currentTouch.dragged.transform.position.y >  gameUIObjects.line.position.y then 
            ...
            if obj ~= playerView.hnhsm_GridTool.gridMo:GetChild(0).gameObject then 
            ...
            obj:SetActive(false);
            this.HNHSM_Logic.needChuPai = false;
        else--没有拖拽到出牌的位置
            obj:SetActive(true);
            if tingTip then 
                this:SetTiPlateMark(playerView.hnhsm_GridTool.gridHand,myData.mahjongs,myData.tingTip,myData);
            end
    ...
    playerView.hnhsm_GridTool:RefreshGridXi(myData.operationCards);
    playerView.hnhsm_GridTool:RefreshGridHand(myData.mahjongs,#myData.mahjongs);
...
function HNHSM_UILayer.OnDoubleClickPlate(obj)
...
function HNHSM_UILayer.OnClickMahjong(obj)
    ...
    -- print("this.HNHSM_Logic.mySeat:"..this.HNHSM_Logic.mySeat);
	if this.lastSelectmahjong then--选择的是同一个，相当于选中，再点击同一个取消选择
		local lastPlate = GetUserData(this.lastSelectmahjong);
        HNHSM_PlayLogic.CheckPlateState(lastPlate,false,this,this.HNHSM_Logic);
        if this.roomData.activeSeat == this.HNHSM_Logic.mySeat then 
             gameUIObjects.playerTingButton.gameObject:SetActive(false);
        end
        local bgFrame = gameUIObjects.playerTingButton:Find("frame");
		bgFrame.gameObject:SetActive(false);
	...
	HNHSM_PlayLogic.CheckPlateState(plate,needTiTip,this,this.HNHSM_Logic);
    if needTiTip then
		--显示提牌
		local tingTip = obj.transform:Find("tingTip");
		if tingTip then
...
function HNHSM_UILayer.OnDragging(obj)
    -- print("OnDragging------------------->"..tostring(obj));

end

function HNHSM_UILayer.OnPressMahJong(obj,state)
...
--填充听牌提示的内容
function HNHSM_UILayer:SetTingScrollView(tingMahjongs)
	...
	for firdex = 1, #tingMahjongs do
		...
		UIEventListener.Get(obj.gameObject).onClick = HNHSM_EventBind.OnClickButtonTingItem;

		--统计张数
		obj:Find("num"):GetComponent("UILabel").text =  HNHSM_PlayLogic.GetTingPaiCount(tingMahjongs[firdex].mahjong,self.HNHSM_Logic,self);
		...
		obj:Find("fan"):GetComponent("UILabel").text  = "";
         ...
         for j=1,#hunPlates do
             if tingMahjongs[firdex].mahjong == hunPlates[j] then
                 ...
                 cardMark:Find("marklabel"):GetComponent("UILabel").text = "赖";
                 break
             end
         end
	end
...
function HNHSM_UILayer:SetNewTingScrollView(tingMahjongs)
    if not tingMahjongs then return end;
...
function HNHSM_UILayer.StartDragMoPai(obj)
    this.originDragPos  = obj.transform.localPosition;
    this.IsMoPaiDrag    = true;
    this.draggingObject = obj;
end

function HNHSM_UILayer.DragEndMoPai(obj)
    this.IsMoPaiDrag = false;
    obj.transform.localPosition = this.originDragPos;
    this.draggingObject = nil;
end

function HNHSM_UILayer.DraggingMoPai(obj)
    obj.transform.localPosition = this.MousePosition();
end

function HNHSM_UILayer.MousePosition()
    local playerView = gameUIObjects.playerViews[this.HNHSM_Logic:GetCorrectUIIndex(this.HNHSM_Logic:GetPlayerViewIndexBySeat(this.HNHSM_Logic.mySeat))];
    ...
    local grid = nil;
	if this.draggingObject == playerView.hnhsm_GridTool.gridMo:GetChild(0).gameObject then--是摸牌那个位置的游戏对象
        grid = playerView.hnhsm_GridTool.gridMo;
	else--是手牌中的某个对象
        grid = playerView.hnhsm_GridTool.gridHand;
    end
...
function HNHSM_UILayer:SendChuPaiMsg(plate,dragObj)
    ...
    --出牌之后隐藏提牌标识
    local playerView = gameUIObjects.playerViews[self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(self.HNHSM_Logic.mySeat))];
    -- print("playerView.hnhsm_GridTool.gridHand:"..tostring(playerView.hnhsm_GridTool.gridHand));
    -- print("playerView.hnhsm_GridTool.gridMo:"..tostring(playerView.hnhsm_GridTool.gridMo));
	self:HideTiMark(playerView.hnhsm_GridTool.gridHand);
    self:HideTiMark(playerView.hnhsm_GridTool.gridMo);
    self.TrusteeshipFsmMachine:Switch(TrusteeShipStates.TrusteeShipEnd);--取消托管状态
end

--玩家摸牌
function HNHSM_UILayer:SetPlayerMoPai(moData)
    ...
    local playerView = gameUIObjects.playerViews[self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(moData.seat))];
    local playerData = self.HNHSM_Logic:GetPlayerDataBySeat(moData.seat);
    --显示当前玩家的摸牌的grid
    playerView.hnhsm_GridTool:SetGridMoVisible(true);
    self:SetDipaiNum(self.roomData.surplus);
    playerView.hnhsm_GridTool:RefreshMoPai(playerData.moMahjong);
    playerView.hnhsm_GridTool:RefreshGridHand(playerData.mahjongs,playerData.mahjongCount);
    if moData.seat == self.HNHSM_Logic.mySeat then 
        gameUIObjects.playerTingButton.gameObject:SetActive(false);
        gameUIObjects.playerTingButton:Find("frame").gameObject:SetActive(false);
        gameUIObjects.newTingOperation.gameObject:SetActive(false);
        local myData = self.HNHSM_Logic:GetPlayerDataBySeat(self.HNHSM_Logic.mySeat);
        self:OperationForbidden(moData.sealPai);
        self:SetTiPlateMark(playerView.hnhsm_GridTool.gridHand,myData.mahjongs,myData.tingTip,myData);
        self:SetTiPlateMark(playerView.hnhsm_GridTool.gridMo,{moData.mahjong},myData.tingTip,myData);
...
--玩家出牌
function HNHSM_UILayer:SetPlayerChuPai(chuData)
    -- print("player.seat"..chuData.seat.." chu pai,plate="..chuData.mahjong.."chuData.sealPai="..tostring(chuData.sealPai));
    local playerView    = gameUIObjects.playerViews[self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(chuData.seat))];
    local playerData    = self.HNHSM_Logic:GetPlayerDataBySeat(chuData.seat);
    local myData        = self.HNHSM_Logic:GetPlayerDataBySeat(self.HNHSM_Logic.mySeat);
    playerView.hnhsm_GridTool:RefreshGridThrow(playerData.paiHe);
    ...
    self:OperationForbidden(myData.sealPai);
    self:SetPlateUIColor(playerView.hnhsm_GridTool.gridHand,{},false);
    if chuData.seat == self.HNHSM_Logic.mySeat then 
        self:HideTiMark(playerView.hnhsm_GridTool.gridHand);
        self:HideTiMark(playerView.hnhsm_GridTool.gridMo);
        playerView.hnhsm_GridTool:SetGridMoVisible(false);
    end
    ...
    if chuData.seat ~= self.HNHSM_Logic.mySeat then --要判断别人出的牌，自己是否能操作，如果自己有操作却没有操作，也要进入托管
        self:CheckPlayerTrustesship(self.roomData.setting.trusteeship);
    end
    if chuData.seat == self.HNHSM_Logic.mySeat then
        self:SetCountDown();
...
--玩家吃牌
function HNHSM_UILayer:SetPlayerChiPai(chiData)
    --1.显示特效
    local realIndex     = self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(chiData.seat));
    local outRealIndex  = self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(chiData.outSeat));
    local playerView    = gameUIObjects.playerViews[realIndex];
    local playerData    = self.HNHSM_Logic:GetPlayerDataBySeat(chiData.seat);
    local outPlayerData = self.HNHSM_Logic:GetPlayerDataBySeat(chiData.outSeat);
    ...
    --4.刷新手牌
    playerView.hnhsm_GridTool:RefreshGridHand(playerData.mahjongs,playerData.mahjongCount);
    if chiData.seat == self.HNHSM_Logic.mySeat then 
        gameUIObjects.newTingOperation.gameObject:SetActive(false);
        gameUIObjects.playerTingButton.gameObject:SetActive(false);
        --5.清除牌的高亮标记
        self:SetPlateUIColor(playerView.hnhsm_GridTool.gridHand,{},false);
        ...
        self:SetTiPlateMark(playerView.hnhsm_GridTool.gridHand,playerData.mahjongs,playerData.tingTip,myData);
...
--玩家碰牌
function HNHSM_UILayer:SetPlayerPengPai(pengData)
    local realIndex     = self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(pengData.seat));
    local outRealIndex  = self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(pengData.outSeat));
    local myRealIndex   = self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(self.HNHSM_Logic.mySeat));
    local playerView    = gameUIObjects.playerViews[realIndex];
    local myPlayerView  = gameUIObjects.playerViews[myRealIndex];
    local playerData    = self.HNHSM_Logic:GetPlayerDataBySeat(pengData.seat);
    local myData        = self.HNHSM_Logic:GetPlayerDataBySeat(self.HNHSM_Logic.mySeat);
    local outPlayerData = self.HNHSM_Logic:GetPlayerDataBySeat(pengData.outSeat);
    local outPlayerView = gameUIObjects.playerViews[outRealIndex];
    --显示特效
    ...
    outPlayerView.hnhsm_GridTool:RefreshGridThrow(outPlayerData.paiHe);
    playerView.hnhsm_GridTool:RefreshGridHand(playerData.mahjongs,playerData.mahjongCount);
    -- print("pengData.seat:"..pengData.seat..",mySeat:"..self.HNHSM_Logic.mySeat);
    if pengData.seat == self.HNHSM_Logic.mySeat then 
        gameUIObjects.newTingOperation.gameObject:SetActive(false);
        gameUIObjects.playerTingButton.gameObject:SetActive(false);
        self:SetPlateUIColor(playerView.hnhsm_GridTool.gridHand,{},false);
        self:SetPlateUIColor(playerView.hnhsm_GridTool.gridMo,{},false);
        self:RefreshOperationSend(myData.operations,myData);
        self:SetTiPlateMark(myPlayerView.hnhsm_GridTool.gridHand,myData.mahjongs,myData.tingTip,myData);
    end
    self:SetActivePlayer(self.roomData);
...
--玩家杠牌
function HNHSM_UILayer:SetPlayerGangPai(gangData)
    --显示特效
    local realIndex     = self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(gangData.seat));
    local myRealIndex   = self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(self.HNHSM_Logic.mySeat));
    local playerView    = gameUIObjects.playerViews[realIndex];
    local playerData    = self.HNHSM_Logic:GetPlayerDataBySeat(gangData.seat);
    local myData        = self.HNHSM_Logic:GetPlayerDataBySeat(self.HNHSM_Logic.mySeat);
    local myPlayerView  = gameUIObjects.playerViews[myRealIndex];
    ...
    if gangData.outSeat then 
        local outPlayerData = self.HNHSM_Logic:GetPlayerDataBySeat(gangData.outSeat);
        local outRealIndex  = self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(gangData.outSeat));
        local outPlayerView = gameUIObjects.playerViews[outRealIndex];
        outPlayerView.hnhsm_GridTool:RefreshGridThrow(outPlayerData.paiHe);
    end
    playerView.hnhsm_GridTool:SetGridMoVisible(false);
    self:SetActivePlayer(self.roomData);
    if gangData.seat == self.HNHSM_Logic.mySeat then 
        gameUIObjects.newTingOperation.gameObject:SetActive(false);
        gameUIObjects.playerTingButton.gameObject:SetActive(false);
        self:SetPlateUIColor(playerView.hnhsm_GridTool.gridHand,{},false);
        self:SetPlateUIColor(playerView.hnhsm_GridTool.gridMo,{},false);
...
--获取杠牌特效字符串
function HNHSM_UILayer:GetGangEffectString(gangType)
...
--玩家补牌
function HNHSM_UILayer:SetPlayerBuPai(buData)
    print("SetPlayerBuPai was called----------------------->");
end

--玩家过牌
function HNHSM_UILayer:SetPlayerGuoPai(guoData)
    print("SetPlayerGuoPai was called------------------------>");
    if guoData.seat == self.HNHSM_Logic.mySeat then 
        local myData        = self.HNHSM_Logic:GetPlayerDataBySeat(self.HNHSM_Logic.mySeat);
        local playerView    = gameUIObjects.playerViews[self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(self.HNHSM_Logic.mySeat))];
        self:OperationForbidden(myData.sealPai);
        self:SetPlateUIColor(playerView.hnhsm_GridTool.gridHand,{},false);--清空牌的颜色
        -- self:HideTiMark(playerView.hnhsm_GridTool.gridHand);
        -- self:HideTiMark(playerView.hnhsm_GridTool.gridMo);
        self:CheckPlayerTrustesship(self.roomData.setting.trusteeship);
    else
...
--玩家胡牌
function HNHSM_UILayer:SetPlayerHuPai(huData)
    -- print("SetPlayerHuPai was called-------------------------->");
    gameUIObjects.operation_send.gameObject:SetActive(false);
    local realIndex     = self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(huData.outSeat));
    local playerView    = gameUIObjects.playerViews[realIndex];
    local playerData    = self.HNHSM_Logic:GetPlayerDataBySeat(huData.outSeat);
    local myPlayerView  = gameUIObjects.playerViews[self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(self.HNHSM_Logic.mySeat))];
    self:SetPlateUIColor(myPlayerView.hnhsm_GridTool.gridHand,{},false);
    self:HideTiMark(myPlayerView.hnhsm_GridTool.gridHand);
    self:HideTiMark(myPlayerView.hnhsm_GridTool.gridMo);
    print("huData.winPlayers.length="..tostring(#huData.winPlayers));
    for i=1,#huData.winPlayers do
        local winRealIndex  = self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(huData.winPlayers[i].seat));
        local winPlayerData = self.HNHSM_Logic:GetPlayerDataBySeat(huData.winPlayers[i].seat);
		...
		function()
            ...
            winPlayerView.hnhsm_GridTool:GridHandPlatesDown(winPlayerData.mahjongs,winPlayerData.mahjongs);	
			--自摸的话，显示手上的牌，因为RefreshGridHandByIndex会隐藏掉手上的牌，需要再显示
			if #huData.winPlayers[i].huItems > 0 then
				if huData.winPlayers[i].selfMo then
                    winPlayerView.hnhsm_GridTool:RefreshMoPai(winPlayerData.moMahjong);
                    winPlayerView.hnhsm_GridTool:MoPaiDown(winPlayerData.moMahjong,winPlayerData.mahjongCount);
				end
			end
		end
        );
...
function HNHSM_UILayer:SetPao(winData)
    ...
    local realIndex = self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(winData.outSeat));
...
function HNHSM_UILayer:GetgameUIObjects()
    return gameUIObjects;
end


--根据背景设置文字颜色
function HNHSM_UILayer:SetGameColor(bgIndex)
...
function HNHSM_UILayer:getAccorColor(color)
	if color == 1 then
...
function HNHSM_UILayer:getNumColor(color)
	if color == 1 then
...
--选择背景
function HNHSM_UILayer:ChooseBG(bgIndex)
...
--设置东南西北旁边的局数数目
function HNHSM_UILayer:SetRoundNum(num)
...
--设置底牌的剩余数目
function HNHSM_UILayer:SetDipaiNum(num)
...
function HNHSM_UILayer:SetInviteVisiable(show)
    ...
    if self.HNHSM_Logic.mySeat == self.roomData.bankerSeat then 
...
--显示出牌的状态，东西南北，哪个该出牌了
--当前该出牌的玩家
function HNHSM_UILayer:ShowWaitOpreatEffect(activeSeat,show)
    ...
    for i=1,self.HNHSM_Logic.totalSize do
        local index = self.HNHSM_Logic:GetPlayerViewIndexBySeat(activeSeat);
        self:SetESWNState(self.HNHSM_Logic:GetCorrectUIIndex(index),show);
    end
    --倒计时
    if show then 
...
--设置倒计时
function HNHSM_UILayer:SetCountDown(trusteeShipTime)
    --如果选择了托管，那么倒计时就已托管的时间开始计时
    if self.roomData.setting.trusteeship > 0 then 
        HNHSM_Tools.CountDown(gameUIObjects.dipai:Find("timer"),(trusteeShipTime and trusteeShipTime or self.roomData.setting.trusteeship),0);
    else
...
--设置等待其他玩家操作的提示
function HNHSM_UILayer:SetWaitOpTip(show,tipString)
...
--玩家禁止操作
function HNHSM_UILayer:OperationForbidden(isForbidden)
...
--提牌标记
function HNHSM_UILayer:SetTiPlateMark(gridHand,mahjongs,tingTips,playerData)
...
--隐藏提牌标识
function HNHSM_UILayer:HideTiMark(grid)
...
function HNHSM_UILayer:SetESWNState(index,show)	gameUIObjects.ActivePlayerESWN[index]:Find('select').gameObject:SetActive(show);
...
function HNHSM_UILayer:SetTip(argShow,argTipString)
...
function HNHSM_UILayer:SetGameTypePos()
...
function HNHSM_UILayer:ShowWaittingUI()
...
function HNHSM_UILayer:HideWaittingUI()
    -- print("HideWaittingUI---------------was called");
    -- print("NeedShowDiPai:"..tostring(HNHSM_PlayLogic.NeedShowDiPai(self.roomData,self.HNHSM_Logic)));
...
--玩家离开房间
function HNHSM_UILayer:SetLeaveRoom()
...
function HNHSM_UILayer:getCardColor()
    return cardColor;
end

function HNHSM_UILayer:getcardText()
    return cardText;
end

function HNHSM_UILayer:GetAudioPrefix()
    return language == 1 and "" or "fy_";
end

function HNHSM_UILayer:OutPlateType()
    return outPlateType;
end

----根据颜色值获取麻将背板图片
function HNHSM_UILayer:getColorCardName(name,colorIndex)
...
--根据用户选择选择麻将的牌面
function HNHSM_UILayer:GetCardTextName(plateIndex,cardTextIndex)
    return tostring(plateIndex);
end

--获取游戏语言选择
function HNHSM_UILayer:GetLanguage()
    return language;
end

--根据用户的选择选择牌面大小
function HNHSM_UILayer:GetCardPlateSize(plateIndex,cardTextIndex)
...
function HNHSM_UILayer:SetPlateColor(throwItemobj,showColor)
    if showColor then
...
--设置玩家出牌展示
function HNHSM_UILayer:SetPlayerChuPaiShow(seat,mahjong)
     local playerView = gameUIObjects.playerViews[self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(seat))];
     playerView.hnhsm_GridTool:RefreshChuPai(mahjong);
end

--设置箭头指向最后一个玩家出的牌
function HNHSM_UILayer:SetLastPlayerChuPaiEffect(seat)
    local playerView = gameUIObjects.playerViews[self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(seat))];
    local playerData = self.HNHSM_Logic:GetPlayerDataBySeat(seat);
...
--刷新玩家吃碰杠补等操作
function HNHSM_UILayer:RefreshOperationSend(operations,playerData)
    ...
    --2.设置相关牌的颜色
    local OpTable, relevantPlates   = HNHSM_PlayLogic.GetRelevantPlates(operations,playerData.mahjongs,playerData.operationCards);
    self.relevantOpTable            = OpTable;
    local realIndex                 = self.HNHSM_Logic:GetCorrectUIIndex(self.HNHSM_Logic:GetPlayerViewIndexBySeat(playerData.seat));
    local playerView                = gameUIObjects.playerViews[realIndex];
    -- print("playerView.hnhsm_GridTool.gridHand.childCount:"..playerView.hnhsm_GridTool.gridHand.childCount..",realIndex:"..realIndex);
    self:SetPlateUIColor(playerView.hnhsm_GridTool.gridHand,relevantPlates,true);
    
end

--设置操作按钮的显示
function HNHSM_UILayer:SetOperationButtons(operations)
...
--设置相关牌的颜色高亮
function HNHSM_UILayer:SetPlateUIColor(grid,relevantPlates,dark)
...
function HNHSM_UILayer:SetCardColor(colorNum)
    cardColor = colorNum;
    self:RefreshAllGrid();
end

function HNHSM_UILayer:SetCardText(textNum)
...
function HNHSM_UILayer:SetLanguage(languageNum)
    language = languageNum;
end

function HNHSM_UILayer:SetOutPlateType(Type)
    print("SetOutPlateType---->"..Type);
    outPlateType = Type;
end


function HNHSM_UILayer:NotifyGameStart()
    if  (self.HNHSM_Logic:GetPlayerDataLength() == self.roomData.setting.size and 
...
--设置玩家的托管
function HNHSM_UILayer:SetPlayerTrustesship(seat,enabled)
    ...
    if seat == self.HNHSM_Logic.mySeat then --进入托管
...
--检查托管的状态
function HNHSM_UILayer:CheckPlayerTrustesship(trusteeShipTime)
    local operations = self.HNHSM_Logic:GetPlayerDataBySeat(self.HNHSM_Logic.mySeat).operations;
    -- print("operations.length="..#operations);
    if  self.roomData.setting.trusteeship > 0 and 
        (#operations > 0 or self.roomData.activeSeat == self.HNHSM_Logic.mySeat) and 
...
function HNHSM_UILayer:CheckPlayerOtherTrustesship(trusteeShipTime,isRoundEndCheck)
...
--初始化托管状态机
function HNHSM_UILayer:InitTrusteeMachine()
...
function HNHSM_UILayer:SetRoomNoExist()
...
--获取游戏中的混牌
function HNHSM_UILayer:GetHunPlates()
    return self.HNHSM_Logic:GetPlayerDataBySeat(self.HNHSM_Logic.mySeat).hunMahjongs;
end
