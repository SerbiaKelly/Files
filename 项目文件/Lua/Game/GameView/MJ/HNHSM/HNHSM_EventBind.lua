...
function this.OnClickButtonSetting(go)
    local sendData      = {};
    sendData.roomData   = HNHSM_Logic.roomData;
    sendData.playerSize = HNHSM_Logic.totalSize;
    sendData.logic      = HNHSM_Logic;
    if HNHSM_Logic.roomData.round > 1 or HNHSM_Logic:IsAllReaded() then 
        sendData.ingame = true;
    else
        sendData.ingame = false;
    end
    PanelManager.Instance:ShowWindow('panelGameSetting_hnhsm', sendData);
...
function this.OnClickOperationButton(go)
    ...
    local myData    = HNHSM_Logic:GetPlayerDataBySeat(HNHSM_Logic.mySeat);
    ...
    if go == gameUIObjects.ButtonHu.gameObject then 
        ...
        local op_mahjong    = HNHSM_PlayLogic.GetOpeartionPlates(myData.operations,uiLayer.hnm_pb.HU)[1];
    ...
    elseif go == gameUIObjects.ButtonGuo.gameObject then 
        ...
        playerView = uiLayer:GetgameUIObjects().playerViews[HNHSM_Logic:GetCorrectUIIndex(HNHSM_Logic:GetPlayerViewIndexBySeat(HNHSM_Logic.mySeat))]
        uiLayer:SetPlateUIColor(playerView.hnhsm_GridTool.gridHand,{},false);
...
function this.OnPressButtonSound(obj,state)
    if HNHSM_Logic.roomData.setting.sendVoiceAllowed then 
        if state == true then 
            gameUIObjects.RecordTiShi.gameObject:SetActive(true);
            HNHSM_Logic:RequestRecord();
        else
            ...
            HNHSM_Logic:ResponseRecord(not (mousePositionY > buttonY + buttonHeight))
...
function this.OnClickButtonXLInvite(go)
	...
	str = str .."划水麻将,";
	str = str ..GetHNHSMRuleText(HNHSM_Logic.roomData.setting,false,true);
	str = str .."房已开好,就等你来！";
	print("分享信息", str);
	local que = HNHSM_Logic.roomData.setting.size - HNHSM_Logic:GetPlayerDataLength();
	local title = roomInfo.roomNumber.."房,"..HNHSM_Logic.roomData.playName..HNHSM_Logic.roomData.setting.size.."缺"..que;
	print("title",title);
	--print("panelLogin.HttpUrl:"..tostring(panelLogin.HttpUrl));
	PlatformManager.Instance:ShareLinkToXL('http://'..panelLogin.HttpUrl..'/share/xl/jionRoom.html?roomId='..roomInfo.roomNumber..'&gameType='..proxy_pb.MJ.."&roomType="..proxy_pb.HSM..'&appType=1', title, str);
end
...
function this.OnClickButtonInvite(go)
	...
	PlatformManager.Instance:ShareLink('http://'..panelLogin.HttpUrl..'/shareRoom.html?appType=1&state=' ..proxy_pb.MJ.."&roomType="..proxy_pb.HSM..roomInfo.roomNumber, title, str, 0);
