...
panelStageClearAll_mj = {}
local this = panelStageClearAll_mj
...
--	四人
	...
	for i = 1, 4 do
		...
		playerSize4[i].ZimoCount 		= playerSize4[i].transform:Find("ScoreGroup/ZimoCount");
		playerSize4[i].ChongpaiCount 	= playerSize4[i].transform:Find("ScoreGroup/ChongpaiCount");
		playerSize4[i].DianpaoCount 	= playerSize4[i].transform:Find("ScoreGroup/DianpaoCount");
		playerSize4[i].QianggangCount 	= playerSize4[i].transform:Find("ScoreGroup/QianggangCount");
...
--	三人和二人
	for i = 1, 3 do
		...
		playerSize3and2[i].ZimoCount 		= playerSize3and2[i].transform:Find("ScoreGroup/ZimoCount");
		playerSize3and2[i].ChongpaiCount 	= playerSize3and2[i].transform:Find("ScoreGroup/ChongpaiCount");
		playerSize3and2[i].DianpaoCount 	= playerSize3and2[i].transform:Find("ScoreGroup/DianpaoCount");
		playerSize3and2[i].QianggangCount 	= playerSize3and2[i].transform:Find("ScoreGroup/QianggangCount");
...
function this.Refresh()
	...
	function SetPlayerInfo(playerObj,playerData)
		playerObj.transform.gameObject:SetActive(true);

		local pData = panelInGamemj.GetPlayerDataBySeat(playerData.seat);
		--1.姓名
		...
		--10.接炮次数
		playerObj.ChongpaiCount:GetComponent("UILabel").text 	= "接炮次数："..playerData.jiePao;
		--11.点炮次数
		playerObj.DianpaoCount:GetComponent("UILabel").text 	= "点炮次数："..playerData.dianPao;
		--12.中鸟次数
		playerObj.QianggangCount:GetComponent("UILabel").text 	= "中鸟个数："..playerData.bird;
		--13.积分
		playerObj.ScoreGet.gameObject:SetActive(playerData.win >= 0);
		playerObj.ScoreLose.gameObject:SetActive(playerData.win < 0);
		if playerData.win >= 0 then
			if playerData.win > 0 then
				playerObj.ScoreGet:GetComponent("UILabel").text = "+"..playerData.win;
			else
				playerObj.ScoreGet:GetComponent("UILabel").text = playerData.win;
			end
		end
		chengJiStr=chengJiStr..'【'..pData.name..'】 '..playerData.win..';'
		playerObj.ScoreLose:GetComponent("UILabel").text = playerData.win;
	...
	if RoundAllData.roomData.setting.size == 4 then
		...
		for i = 1, 4 do
			...
			SetPlayerInfo(playerSize4[i],player);
			--托管标志
	...
	elseif RoundAllData.roomData.setting.size == 3 then
		...
		for i = 1, #RoundAllData.data.players do
			local player = RoundAllData.data.players[i];
			SetPlayerInfo(playerSize3and2[i],player);
			--托管标志
	...
	elseif RoundAllData.roomData.setting.size == 2 then
		...
		for i = 1, #RoundAllData.data.players do
			...
			if needShowMidPanel then
				SetPlayerInfo(playerSize3and2[this.GetPlayerIndex(RoundAllData.roomData.setting.size,i)],player);
