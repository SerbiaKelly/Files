dzm_pb 			= require("dzm_pb");
DZAHM_PlayLogic = {};
local this 		= DZAHM_PlayLogic;


--是否需要显示东南西北方位展示的UI
function this.NeedShowDiPai(roomData,logic)
    if roomData.state == dzm_pb.GAMING then--如果正在游戏中，那么肯定是要显示
		return true;
	else
		if roomData.state == dzm_pb.READYING then--如果是准备状态
...
function this.GetRelevantPlates(operations,handMahjongs,operationCards)
	...
	for i=1,#operations do
		if operations[i].operation == dzm_pb.CHI then 
		...
		elseif operations[i].operation == dzm_pb.PENG then 
		...
		elseif operations[i].operation == dzm_pb.GANG or operations[i].operation == dzm_pb.BU then 
			-- print("path--------------------->dzm_pb.GANG");
...
function this.FindAllPengPlates(handMahjongs,plates)
	...
	local relevantPlates = {};
	for i=1,#plates do
		table.insert( canPenTable, {type = dzm_pb.PENG,mahjong = plates[i],mahjongs={plates[i],plates[i],plates[i]}} );
		for j=1,2 do
			table.insert( relevantPlates, plates[i] );
		end	
	end
...
function this.FindAllBuGangPlates(handMahjongs,plates,op_type,operationCards)
	...
	for i=1,#plates do
		for j=1,#operationCards do
			...
			-- print("path--------->3,operation_type == dzm_pb.SORT_PENG:"..tostring(operation_type == dzm_pb.SORT_PENG)..",plates[i] == op_cards[1]:"..tostring(plates[i] == op_cards[1])..",(op_type == dzm_pb.GANG:"..tostring(op_type == dzm_pb.GANG)..",(op_type == dzm_pb.BU:"..tostring(op_type == dzm_pb.BU));
			--这里解释一下，(operation_type == dzm_pb.SORT_PENG and plates[i] == op_cards[1]) 表示必须要从碰牌里面去选，碰牌才能
			--杠或者补，(op_type == dzm_pb.GANG or op_type == dzm_pb.BU)表示必须是补和杠才能这样做
			if (operation_type == dzm_pb.SORT_PENG and plates[i] == op_cards[1]) and (op_type == dzm_pb.GANG or op_type == dzm_pb.BU)   then 
...
function this.PrintOP(op)
	local opstring = "";
	if op == dzm_pb.GUO then
		opstring = "过";
	elseif op == dzm_pb.CHI then
		opstring = "吃";
	elseif op == dzm_pb.PENG then
		opstring = "碰";
	elseif op == dzm_pb.BU then
		opstring = "补";
	elseif op == dzm_pb.GANG then
		opstring = "杠";
	elseif op == dzm_pb.HU then
		opstring = "胡";
	elseif op == dzm_pb.LAO then
		opstring = "捞";
	else
		opstring = "未知操作类型";
	end
...
function this.CheckPlateState(plate, showColor,uiLayer,logic)
	...
	for firdex = 1, logic.totalSize do
		...
		if playerView.dzahm_GridTool.gridThrow.childCount > 0 then
			for secdex = 1, playerView.dzahm_GridTool.gridThrow.childCount do
				local throwItem = playerView.dzahm_GridTool.gridThrow:GetChild(secdex-1);
