    ...
    if roomData.state == hnm_pb.GAMING then--如果正在游戏中，那么肯定是要显示
		...
		if roomData.state == hnm_pb.READYING then--如果是准备状态
			if roomData.round > 1 then --之前刚刚打完一局，进入下一句的等待中
				...
				if logic:GetPlayerDataLength() == roomData.setting.size and logic:IsAllReaded() then--游戏开启了蹲拉跑
					return true;
				else
					return false;
				end
...
function this.CheckPlateState(plate, showColor,uiLayer,logic)
	...
	for firdex = 1, logic.totalSize do
		...
		if playerView.hnhsm_GridTool.gridThrow.childCount > 0 then
			for secdex = 1, playerView.hnhsm_GridTool.gridThrow.childCount do
				local throwItem = playerView.hnhsm_GridTool.gridThrow:GetChild(secdex-1);
