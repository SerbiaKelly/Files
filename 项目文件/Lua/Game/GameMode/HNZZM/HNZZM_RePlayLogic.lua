require("GameLogic");
require("HNZZM_RePlayUILayer");
require("FsmMachine");
local json          = require("json");
local hnm_pb        = nil;
HNZZM_RePlayLogic   = {};
HNZZM_RePlayLogic   = class(GameLogic);
local this          = nil;
local message       = nil;
...
function HNZZM_RePlayLogic:ctor(gameView, game_pb, gameType, obj)
    this            = self;
    hnm_pb          = game_pb;
    replayUILayer   = HNZZM_RePlayUILayer.New(obj,this,hnm_pb);
end



function HNZZM_RePlayLogic:GetUILayer()
    return replayUILayer;
end
function HNZZM_RePlayLogic:GetMaxPlayerSize()
    return c_MaxSize;
end
...
function HNZZM_RePlayLogic:GetCorrectUIIndex(index)
...
function HNZZM_RePlayLogic:GetPlayerViewBySeat(seat)
...
--玩家摸牌
function HNZZM_RePlayLogic:OnPlayerMoPai(data)
...
--玩家出牌
function HNZZM_RePlayLogic:OnPlayerChuPai(data)
    local playerData        = self:GetPlayerDataBySeat(data.seat);
...
--玩家吃牌
function HNZZM_RePlayLogic:OnPlayerChiPai(data)
    self.roomData.activeSeat = data.seat;
    local playerData = self:GetPlayerDataBySeat(data.seat);
...
--玩家碰牌
function HNZZM_RePlayLogic:OnPlayerPengPai(data)
    local playerData            = self:GetPlayerDataBySeat(data.seat);
    self.roomData.activeSeat    = data.seat;
...
--玩家杠牌
function HNZZM_RePlayLogic:OnPlayerGangPai(data)
    self.roomData.activeSeat    = data.seat;
    local playerData            = self:GetPlayerDataBySeat(data.seat);
...
--玩家补牌(呼市麻将没有补的操作)
function HNZZM_RePlayLogic:OnPlayerBuPai(data)
    replayUILayer:SetPlayerBuPai(data);
end

--玩家过牌
function HNZZM_RePlayLogic:OnPlayerGuoPai(data)
    print("OnPlayerGuoPai was called------------->");
    replayUILayer:SetPlayerGuoPai(data);
end

--玩家胡牌
function HNZZM_RePlayLogic:OnPlayerHuPai(data)
    replayUILayer:SetPlayerHuPai(data);
end



function HNZZM_RePlayLogic:AutoRegisterPlayerDatas(playerDatas,resetView)
...
--开局倒计时自动解散的
function HNZZM_RePlayLogic:CheckAutoDissolutionState()

end

function HNZZM_RePlayLogic:OnJoinRoom(data)
...
function HNZZM_RePlayLogic:GetInitSurplus(roomData)
...
--玩家加入
function HNZZM_RePlayLogic:OnPlayerJoin(data)
    print("-----------------OnPlayerJoin---------------")
...
function HNZZM_RePlayLogic:OnInitView(data)
    self:RemoveAllPlayerView(true);
end

function HNZZM_RePlayLogic:OnPlayerLeave(seat)
    print("座位号："..seat.."玩家，离开房间")
    replayUILayer:RefreshGpsView();
end


function HNZZM_RePlayLogic:OnPlayerDisconnected(seat) 
    print("座位号："..seat.."玩家，断开连接") 
end

function HNZZM_RePlayLogic:OnGameRoundStart(data)
...
function HNZZM_RePlayLogic:OnGameRoundEnd(data)
...
--清除一些游戏状态
function HNZZM_RePlayLogic:ClearGameStates()
    --1.清除准备状态
...
function HNZZM_RePlayLogic:GetCorrectRound()
...
function HNZZM_RePlayLogic:OnGameRoundOver(data)
...
function HNZZM_RePlayLogic:OnGameDestroy(data)
    print("-----------------OnGameDestroy-----------------")
...
function HNZZM_RePlayLogic:RestoreGameScene()
    replayUILayer:RefreshAllUI();
end

function HNZZM_RePlayLogic:ResetRoundStartState()
...
function HNZZM_RePlayLogic:ResetGameState()
    self:ForeachPlayerDatas(function(playerView,_) playerView:ResetGameState() end)
end


function HNZZM_RePlayLogic:ChangePaiSize(hnzzmPaiSize)
    self.hnzzmPaiSize = hnzzmPaiSize;
    replayUILayer:ChangePaiSize(hnzzmPaiSize);
end

function HNZZM_RePlayLogic:IsGaming(logic, roomData)
    return logic:IsAllReaded() or not IsGameReadState(self.roomData) or self.roomData.round > 1
end


--刷新庄家位置
function HNZZM_RePlayLogic:OnRefreshBanker(data)
...
--设置玩家抓鱼分数
function HNZZM_RePlayLogic:OnSetPlayerFish(data)
...
--设置玩家分数
function HNZZM_RePlayLogic:OnPlayerScore(data)
    replayUILayer:SetPlayerScore(data);
end

--设置玩家跑分
function HNZZM_RePlayLogic:OnPlayerSelectPao(data)
...
--设置房间混牌
function HNZZM_RePlayLogic:SetHun(data)
...
--播放记录
function HNZZM_RePlayLogic:OnPlayerPlay(record)
...
function HNZZM_RePlayLogic:InitPlayerExtraInfo(record)
